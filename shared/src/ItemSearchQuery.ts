export class ItemSearchQuery {
    ...
    public getKeywords(): string[] {
        return !this.keywords
               ? []
               : this.keywords.split(ItemSearchQuery.keywordsSeparator);
    }

    public toUrl(): string {
        return `${ItemSearchQuery.freeTextParamName}=${this.getFullText()}&${ItemSearchQuery.keywordsParamName}=${this.getKeywords().join(ItemSearchQuery.keywordsSeparator)}`;
    }
}